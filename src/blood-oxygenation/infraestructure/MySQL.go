...
func (mysql *MySQL) SaveBloodOxygenation(esp32ID string, tiempo string, ir int32, red int32) error {
	...
	if rowsAffected == 1 {
		log.Printf("[MySQL] - Saturación de oxígeno en sangre guardada correctamente: Esp32ID:%s Saturación:%f", esp32ID, float64(ir+red)/2.0)
	} else {
...
func (mysql *MySQL) GetAll() ([]domain.BloodOxygenation, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&bloodOxygen.ID, &bloodOxygen.ESP32ID, &bloodOxygen.Timestamp, &bloodOxygen.IR, &bloodOxygen.Red); err != nil {
