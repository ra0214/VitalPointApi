...
func (cu *CreateBodyTemperature) Execute(esp32ID string, temperature float64) error {
	err := cu.db.SaveBodyTemperature(esp32ID, temperature)
	if err != nil {
		return err
	}

	bodyTemperature := domain.NewBodyTemperature(esp32ID, temperature)
